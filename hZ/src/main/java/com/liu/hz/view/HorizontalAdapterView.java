 ...
 *
 * <p>
 * See {@link com.liu.hz.view.ListView}, {@link android.widget.GridView}, {@link android.widget.Spinner} and
...
public abstract class HorizontalAdapterView<T extends Adapter> extends ViewGroup {
    ...
    // Unhide method from ViewGroup (j.m.)
    protected boolean mClipToPadding = false;
    // Unhide method from ViewGroup (j.m.)
    protected boolean mDisallowIntercept = false;
    ...
    private void setImportantAccessibility() {
    ...
    public void setEmptyView(View emptyView) {
        ...
        getImportantForAccessibility(emptyView);
    ...
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN)
    private void getImportantForAccessibility(View emptyView) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            // If not explicitly specified this view is important for accessibility.
            if (emptyView != null
    ...
    void handleDataChanged() {
        ...
        //TODO: Im not sure why this was there (jm)
//        notifyAccessibilityStateChanged();
    ...
    protected AccessibilityManager getAccessibilityManager() {
...
//    @ViewDebug.ExportedProperty(category = "layout")
    ...
    // Unhide ViewGroup method (j.m.)
    public void offsetChildrenTopAndBottomUnhide(int offset) {
    ...
    // Unhide ViewGroup method (j.m.)
    public void offsetChildrenLeftAndRightUnhide(int offset) {
        final int count = getChildCount();
    ...
    // Unhide View method (j.m.)
    protected void invalidateParentCachesUnhide() {
        final ViewParent parent = getParent();
    ...
    // Unhide View method (j.m.)
    @TargetApi(Build.VERSION_CODES.ICE_CREAM_SANDWICH)
    protected boolean performButtonActionOnTouchDownUnhide(MotionEvent event) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            if ((event.getButtonState() & MotionEvent.BUTTON_SECONDARY) != 0) {
                if (showContextMenuUnhide(event.getX(), event.getY(), event.getMetaState())) {
                    return true;
                }
            }
        }
    ...
    // Unhide View/ViewGroup method (j.m.)
    public static void dispatchStartTemporaryDetach(View view) {
        // TODO its very hard to implement those (j.m.)
...
//        clearDisplayList();
    ...
    // Unhide View method (j.m.)
    protected float getVerticalScrollFactorUnhide() {
    ...
    // Unhide View method (j.m.)
    protected float getHorizontalScrollFactorUnhide() {
